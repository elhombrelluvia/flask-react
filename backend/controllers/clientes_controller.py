...
def validar_campos_cliente(data):
    ...
    """
    campos_obligatorios = ["rut", "nombre", "apellido", "pedidos_activos", "historial_pedidos"]
    ...
    if not isinstance(data["pedidos_activos"], int) or data["pedidos_activos"] < 0:
        return False, "pedidos_activos debe ser un entero no negativo"
...
def obtener_clientes_activos():
    """
    Obtiene todos los clientes con pedidos activos.
    """
    clientes = [
        serializar_cliente(cliente)
        for cliente in clientes_collection.find({"pedidos_activos": {"$gt": 0}})
    ]
    return jsonify(clientes), 200
